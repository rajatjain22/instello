...
export default function DeleteConfirmModel({
  ...
  return (
    <ModelBox isOpen={isOpen} onClose={onClose}>
      ...
      <div
        className="bg-white flex flex-col text-black gap-3 sm:px-4 sm:py-3 p-2.5"
      >
        Are you sure you want to delete this post?
      </div>

      <div className="sm:px-4 sm:py-3 p-2.5 border-t border-gray-100 flex items-center justify-end gap-1 dark:border-slate-700/40">
        <button
        ...
        {loading ? (
          <button
            className="text-sm rounded-full py-1.5 px-3.5 bg-secondery text-[red] cursor-not-allowed"
            disabled
          >
            <svg
              aria-hidden="true"
          ...
          <button
            className="text-sm rounded-full py-1.5 px-3.5 bg-secondery text-[red]"
