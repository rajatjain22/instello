...
export default function CommentModel({
  ...
  const fetchComments = async () => {
    try {
      const res = await fetch(`/api/post/comment/${post._id}?page=${page}`);
      if (!res.ok) {
      ...
      setComment((presVal) => ({
        ...presVal,
        comments: [...presVal.comments, ...resJson.data],
      }));
  ...
  const handleSubmitComment = useCallback(
    ...
    [comment.value, post._id, handleCommentModel, userDetails]
  );
    ...
    <ModelBox isOpen={isOpen} onClose={onClose}>
      ...
      >
        {comment.pageLoading ? (
          ...
          <InfiniteScroll
            dataLength={comment.comments.length}
            next={fetchComments}
            hasMore={hasMore}
            loader={
              <>
            ...
            {comment.comments.map((val, index) => (
              <div className="flex items-start gap-3 relative" key={index}>
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-0.5 font-normal whitespace-pre-wrap">
                    {val.comment}
                  </p>
                </div>
              </div>
